from mura.versioning import CURRENT_PIPELINE_VERSIONS, get_pipeline_versions


def test_pipeline_versions_are_explicit_and_copy_safe() -> None:
    versions = get_pipeline_versions()

    assert versions.pipeline == "mura-core-v0.10.0"
    assert versions.domain_schema == "domain-v2"
    assert versions.cleaner_prompt == "cleaner-v2-untrusted-transcript"
    assert versions.extractor_prompt == "extractor-v4-provenance-safe"
    assert versions.extractor_repair_prompt == "extractor-repair-v2-bounded-untrusted"
    assert versions.evidence_rules == "claim-evidence-v3-layered-provenance+bounded-coreference-v2"
    assert versions.resolver == "mention-resolver-v2-cross-recording"
    assert (
        versions.archive_schema == "archive-claim-ledger-v1+conflict-decisions-v1+generic-claims-v1"
    )
    assert versions.materializer == "family-materializer-v3-graph-and-profiles"
    assert versions.evaluator == "core-evaluator-v2-provenance-safety+entity-resolution-v1"
    assert versions.benchmark_schema == "benchmark-v2+entity-resolution-benchmark-v1"
    assert versions is not CURRENT_PIPELINE_VERSIONS
