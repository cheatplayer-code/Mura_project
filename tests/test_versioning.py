...
def test_pipeline_versions_are_explicit_and_copy_safe() -> None:
    ...
    assert versions.pipeline == "mura-core-v0.12.0"
    assert versions.domain_schema == "domain-v4-focused-narratives"
    ...
    assert (
        ...
        == "claim-evidence-v4-ordered-factual-support+bounded-coreference-v2"
    )
    ...
    assert versions.evaluator == "core-evaluator-v4-events-stories+entity-resolution-v1"
    assert versions.benchmark_schema == "benchmark-v4-events-stories+entity-resolution-benchmark-v1"
