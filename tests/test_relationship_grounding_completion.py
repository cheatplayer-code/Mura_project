...
def test_segment_boundary_preserves_coreference_and_real_provenance() -> None:
    ...
    assert all(
        item.segment_id in {"seg_001", "seg_002"}
        for item in completed.evidence_spans
    )
...
def test_third_sentence_and_oversized_context_are_not_used() -> None:
    ...
    for transcript in transcripts:
        contexts = select_relationship_grounding_contexts(
            ...
            people=people,
            speaker_name="Күләш",
            resolved_antecedent_ids={"mention_erлан"},
        )
        assert contexts == []
...
def test_only_open_conflict_preserves_review_candidates() -> None:
    ...
    assert all(
        item.conflict_ids == ["conflict_relationship"]
...
def _pipeline_with_open_conflict() -> PipelineResult:
    ...
    evidence = EvidenceSpan(
        ...
        mention_ids=["mention_erлан", "mention_dinara"],
    )
    people = [
        PersonMention(
            mention_id="mention_erлан",
            name="Ерлан",
            category="family_member",
            source_segment_ids=["seg_001"],
    ...
    relationship = RelationshipClaim(
        ...
        subject_mention_id="mention_erлан",
    ...
    return PipelineResult(
        ...
        cleaned_transcript=CleanerResult(
            readable_segments=[
                ReadableSegment(segment_id="seg_001", text="Ерлан Динара")
            ],
