...
def _candidate_name_signals(
    ...
    if normalized_primary and normalized_primary in aliases:
        signals.append(
            ResolutionSignal(
                rule_id="resolution.name.established_alias.v2",
                kind=ResolutionSignalKind.ESTABLISHED_ALIAS,
                detail=f"mention primary name matches an established alias of {person.person_id}",
                person_id=person.person_id,
            )
        )
...
def _candidate_state(mention: PersonMention, profile: KnownPersonProfile) -> _CandidateState | None:
    ...
    if not name_signals:
        ...
        name_signals = [
            ResolutionSignal(
                ...
                kind=ResolutionSignalKind.CANONICAL_NAME,
                detail="a normalized mention surface matches a canonical name or established alias",
    ...
    if mention_generation is not None and profile.generation is not None:
        if mention_generation == profile.generation:
            state.supporting.append(
                ResolutionSignal(
                    rule_id="resolution.context.generation_match.v1",
                    kind=ResolutionSignalKind.GENERATION,
                    detail=f"relative generation agrees at {mention_generation}",
                    person_id=profile.person.person_id,
                )
            )
        else:
    ...
    for relationship in extraction.relationship_claims:
        ...
        state.supporting.append(
            ResolutionSignal(
                ...
                detail=(
                    f"validated {relationship.relationship_type.value} claim agrees with archive "
