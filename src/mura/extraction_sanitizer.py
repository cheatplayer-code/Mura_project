...
def _parse_items(
    *,
    ...
    for raw_item in raw_items:
        ...
        resolved_id = getattr(item, id_field)
...
def sanitize_extraction_output(
    *,
    ...
    """Salvage valid unreviewed claims and quarantine malformed individual objects.

    The ASR transcript and caller-provided speaker identity are authoritative. A single bad
    relationship, description, event, story, or question must not destroy the rest of a useful
    extraction. Invalid objects are excluded and returned as diagnostics for a future review
    queue. The final result still passes the same strict validator.
    """
    issues: list[ExtractionIssue] = []
